...
youth <- read_csv("Data/Data-Raw/YRBS2015.csv", skip = 1,
                  na = c("N/A","."), 
                  col_types = list("AgeCat" = col_number(),
                                   "Grade" = col_number(),
                                   "Gender" = col_character(),
                                   "Height in meters" = col_double(), 
                                   "Weight in kilograms" = col_double(), 
                                   "Body Mass Index" = col_double(), 
                                   "BMI percentile" = col_integer(), 
                                   "Fruit eating" = col_character(),
                                   "Salad eating" = col_character(), 
                                   "Other vegetable eating" = col_character(),
                                   "Soda drinking" = col_character(), 
                                   "Breakfast eating" = col_number(), 
                                   "Physical activity >= 5 days" = col_number(),
                                   "Television watching" = col_character(), 
                                   "Sports team participation" = col_number(),
                                   "Perception of weight" = col_factor(levels = c("Very underweight", "Slightly underweight","About the right weight","Slightly overweight","Very overweight")),
                                   .default = col_skip()
                                   ))
```
...
> The data seems to be in characters rather than numeric
...
#| eval: false
youth_clean <- clean_names(youth)
write_csv(youth_clean, "Data/Data-Clean/youth_clean.csv")
```

Why do we keep the code for the data cleaning but set the code chunks to `eval: false`?

> We only want the clean code.
...
> The first graph shows the number of missing data in each specific variables while the second graph shows the overlaping of missing data with the combination of different variables.

How does the missingness of the data impact what variables you might choose to analyze? Why?

> Missingness of data impacts what variables one wants to study as the more missing data there is with a variable, the more we cannot be able to use the data on a study/analysis. It limits the choices of variables we want to use to analysis and study.
...
youth_clean |>  #instead of data = youth_clean in ggplot we can "pipe" our data  
  ...
  ggplot() + 
  geom_point(mapping = aes(x = height_in_meters,
                           y = weight_in_kilograms,
                           color = perception_of_weight,
                           alpha = 0.25)) +
  geom_smooth(method = lm, 
              mapping = aes(x = height_in_meters,
                            y = weight_in_kilograms)) +
  labs(title = "Student's Weight(kg) to Height(m)",
       x = "Height in Meters",
       y = "Weight in Kilograms") +
  facet_wrap(grade ~ gender) +
  theme_bw() +
  theme(legend.position = "bottom")

#ggsave("weight-perception.png")
```
